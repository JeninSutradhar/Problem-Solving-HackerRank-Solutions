...
int getTotalX(int a_count, int* a, int b_count, int* b) {
    // Compute LCM of all elements in array a
    int current_lcm = a[0];
    for (int i = 1; i < a_count; i++) {
        current_lcm = lcm(current_lcm, a[i]);
    }
    
    // Compute GCD of all elements in array b
    int current_gcd = b[0];
    for (int i = 1; i < b_count; i++) {
        current_gcd = gcd(current_gcd, b[i]);
    }
    
    // Count the number of integers that are multiples of LCM and divide GCD
    int count = 0;
    for (int i = current_lcm; i <= current_gcd; i += current_lcm) {
        if (current_gcd % i == 0) {
            count++;
        }
    }
